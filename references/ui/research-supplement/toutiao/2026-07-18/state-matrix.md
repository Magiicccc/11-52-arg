...
|---|---|---|---|
| 新闻信息流 | direct_real | TT-FEED-01 | 按直接截图复刻；剧情只替换内容字段。 |
| 热榜 | direct_real | TT-HOT-01 | 按直接截图复刻；剧情只替换内容字段。 |
| 文章详情 | system_inherited | — | 沿用同App已有结构，并继承本包iOS系统键盘/弹层/列表组件。 |
| 评论区 | derived_real | — | 只从本App直接截图中的导航、字体、间距、卡片、按钮和弹层组件组合；禁止引用其他App风格。 |
| 搜索/AI搜索 | direct_real | TT-AI-01 | 按直接截图复刻；剧情只替换内容字段。 |
