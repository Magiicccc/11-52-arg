...
export function CalendarApp() {
  ...
  return <UtilityShell appId="app.calendar" className="calendar-real" title={view==="detail"?"日程详情":"日历"} view={view} onBack={()=>setView("home")}>
    {view==="home"?<>
      <div className="calendar-month-head"><button>‹</button><b>2026年7月</b><button>›</button></div>
      <div className="calendar-weekdays">{["日","一","二","三","四","五","六"].map(day=><span key={day}>{day}</span>)}</div>
      <div className="calendar-month">{Array.from({length:35},(_,index)=>{const day=index-2;return <button className={day===15?"selected":day<1||day>31?"muted":""} key={index} onClick={()=>emit("app.calendar.date.selected","app.calendar",{day})}>{day>0&&day<=31?day:""}</button>})}</div>
      <section className="calendar-agenda"><header><b>7月15日 星期三</b><button>＋</button></header>{calendarEvents.map((event,index)=><button data-testid={index===0?"app-effective-action":undefined} key={event.id} onClick={()=>open(event)}><time>{event.time}</time><i/><span><b>{event.title}</b><small>{event.note}</small></span></button>)}</section>
...
export function NeteaseMusicApp() {
  ...
  return <UtilityShell appId="app.netease_music" className="music-real" title={view==="player"?"正在播放":view==="playlist"?"夜路":view==="history"?"最近播放":"网易云音乐"} view={view} onBack={()=>setView("home")} footer={view!=="player"?<nav className="music-nav"><button className="active">发现</button><button>播客</button><button>我的</button></nav>:undefined}>
    {view==="home"&&<>
      <header className="music-profile"><span>川</span><div><b>川流档案</b><small>黑胶会员</small></div><button>搜索</button></header>
      <section className="music-shortcuts"><button onClick={()=>go("playlist")} data-testid="app-effective-action"><i>歌</i><b>夜路</b><small>4 首</small></button><button onClick={()=>go("history")}><i>历</i><b>最近播放</b><small>87 首</small></button><button><i>播</i><b>播客</b><small>城市与记忆</small></button></section>
    ...
    {view==="player"&&<section className="music-player"><div className="record-art"><i/></div><h1>雨停之后</h1><p>川流档案</p><input aria-label="播放进度" type="range" min="0" max="100" defaultValue="38"/><div><button>上一首</button><button className="play" onClick={()=>{setPlaying(value=>!value);emit("content.item.interacted","music.track.01",{action:"playback",playing:!playing})}}>{playing?"Ⅱ":"▶"}</button><button>下一首</button></div><button onClick={()=>go("history")}>查看播放历史</button></section>}
  </UtilityShell>;
}
...
export function WechatReadingApp() {
  ...
  return <UtilityShell appId="app.wechat_reading" className="reading-real" title={view==="reader"?"档案整理方法":view==="notes"?"划线与批注":"微信读书"} view={view} onBack={()=>setView("home")} footer={view==="home"?<nav className="reading-nav"><button className="active">书架</button><button>听书</button><button>发现</button><button>我</button></nav>:undefined}>
    {view==="home"&&<><header className="reading-head"><h1>书架</h1><button>搜索</button></header><div className="reading-summary"><b>9 本书</b><span>本周阅读 2小时16分</span></div><div className="bookshelf">{readingBooks.map((book,index)=><button data-testid={index===0?"app-effective-action":undefined} key={book} onClick={()=>{go("reader");emit("content.item.opened","reading.book.01",{book})}}><span className={`book-cover tone-${index}`}>{book.slice(0,4)}</span><b>{book}</b><small>{index===0?`${progress}%`:"未读"}</small></button>)}</div></>}
...
export function Railway12306App() {
  ...
  return <UtilityShell appId="app.railway12306" className="rail-real" title={view==="order"?"订单详情":view==="trips"?"我的行程":view==="passenger"?"乘车人":"铁路12306"} view={view} onBack={()=>setView("home")} footer={view==="home"?<nav className="rail-nav"><button className="active">首页</button><button onClick={()=>go("trips")}>行程</button><button>订单</button><button>我的</button></nav>:undefined}>
    {view==="home"&&<><header className="rail-hero"><span>铁路出行服务</span><b>火车票</b></header><section className="rail-query"><div><button>天津</button><i>⇄</i><button>杭州</button></div><p>7月18日 周六</p><button onClick={()=>emit("app.search.submitted","app.railway12306",{type:"train"})}>查询车票</button></section><div className="rail-tools">{["车站大屏","乘车人","正晚点","温馨服务"].map((label,index)=><button key={label} onClick={()=>index===1&&go("passenger")}><i>{label.slice(0,1)}</i>{label}</button>)}</div><section className="rail-current"><h2>我的行程</h2><button data-testid="app-effective-action" onClick={()=>go("order")}><small>历史订单 · 已完成</small><b>上海虹桥 → 杭州东</b><span>2023年9月17日 · 2位</span></button></section></>}
    ...
    {view==="passenger"&&<section className="rail-passengers"><h2>乘车人</h2><button><span>沈</span><div><b>沈川</b><small>成人 · 已核验</small></div></button><button><span>我</span><div><b>本人</b><small>成人 · 已核验</small></div></button></section>}
  </UtilityShell>;
}
...
export function WeatherApp() {
  ...
  return <UtilityShell appId="app.weather" className="weather-real" title={view==="cities"?"地点":"杭州"} view={view} onBack={()=>setView("home")}>
    {view==="home"?<div className="weather-page"><header><button onClick={()=>setView("cities")}>地点</button><span>我的位置</span></header><h1>33°</h1><p>阵雨 · 最高33° 最低27°</p><section><b>未来一小时可能有阵雨</b><div className="hourly">{hourly.map(value=><span key={value}>{value}</span>)}</div></section><section><b>7日天气预报</b>{forecast.map((value,index)=><button data-testid={index===0?"app-effective-action":undefined} key={value} onClick={()=>emit("content.item.opened","weather.forecast.01",{day:index})}><span>{value.split(" ")[0]}</span><i>{index%3===0?"阵雨":"多云"}</i><strong>{value.slice(3)}</strong></button>)}</section></div>:<section className="weather-cities"><h1>天气</h1><input placeholder="搜索城市"/><button onClick={()=>setView("home")}><div><b>我的位置</b><span>杭州 · 阵雨</span></div><strong>33°</strong></button><button><div><b>天津</b><span>多云</span></div><strong>31°</strong></button></section>}
  </UtilityShell>;
}
...
export function ClockApp() {
  ...
  return <UtilityShell appId="app.clock" className="clock-real" title={tab} view="home" onBack={()=>undefined} footer={<nav className="clock-tabs">{["世界时钟","闹钟","秒表","计时器"].map(item=><button className={tab===item?"active":""} key={item} onClick={()=>{setTab(item);emitView(emit,"app.clock",item)}}>{item}</button>)}</nav>}>
    {tab==="世界时钟"&&<section className="world-clocks"><h1>世界时钟</h1><button><b>天津</b><strong>15:52</strong><small>今天</small></button><button><b>杭州</b><strong>15:52</strong><small>今天</small></button></section>}
    {tab==="闹钟"&&<section className="alarm-list"><header><button>编辑</button><h1>闹钟</h1><button>＋</button></header>{alarms.map(([time,label],index)=>{const active=state.world.flags[`ui.clock.alarm.${index}`]!==false;return <button data-testid={index===0?"app-effective-action":undefined} key={time} onClick={()=>{setUiFlag(`clock.alarm.${index}`,!active);emit("content.item.interacted","clock.alarm.01",{index,active:!active})}}><div><strong>{time}</strong><span>{label}</span></div><i className={active?"on":""}/></button>})}</section>}
