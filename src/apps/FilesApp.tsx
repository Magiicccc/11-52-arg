  ...
  if(item){
    ...
    return <AppChrome title={b.name??"文件"} actions={<button onClick={goBack}>完成</button>}>
      ...
      {item.id==="file.417_index"&&a3Started&&!state.story.completedSceneIds.includes("A3-01")&&<section className="coordinate-workbench" data-testid="coordinate-workbench">
        <header><b>A3-01 · 组合坐标</b><small>REFERENCE-GAP · 不显示或猜测真实经纬度</small></header>
        ...
        <p>半径：80 米 · 校验序列：4-1-7 · 输出：LOC_RIVER_EDU_OLD_01</p>
      ...
      {item.id==="file.417_index"&&state.story.completedSceneIds.includes("A3-01")&&<section className="reference-gap-card" data-testid="coordinate-result"><b>LOC_RIVER_EDU_OLD_01</b><span>生产令牌已建立；现实地点仍未绑定。</span></section>}
    </AppChrome>;
  }
