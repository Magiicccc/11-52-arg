...
export function PhotosApp(){
  ...
  if(selected==="a3.site.photos"&&siteItem){
    ...
    if(slot) return <AppChrome title={slot.file} actions={<button onClick={()=>setSiteSlot(null)}>十二张</button>}>
      <div className="photo-detail">
        <img src={body.placeholderSrc} alt={`${slot.slot} temporary placeholder`}/>
    ...
    return <AppChrome title="沈川现场照片" actions={<button onClick={()=>setSelected(null)}>所有照片</button>}>
      ...
      <div className="photo-grid a3-photo-grid">{body.slots.map(slotValue=><button data-testid={`site-photo-${slotValue.slot}`} key={slotValue.slot} onClick={()=>setSiteSlot(slotValue.slot)}><img src={body.placeholderSrc} alt="temporary"/><span>{slotValue.slot}<br/>{slotValue.subject}</span></button>)}</div>
  ...
  if(selected==="a3.site.video"&&videoItem){
    ...
    return <AppChrome title="现场视频 · 26.4秒" actions={<button onClick={()=>setSelected(null)}>所有照片</button>}>
      <div className="video-contract" data-testid="a3-site-video">
        <img src={body.placeholderSrc} alt="temporary video placeholder"/>
  ...
  return <AppChrome title="照片" actions={<button data-testid="app-effective-action" onClick={()=>setSelectMode(v=>!v)}>{selectMode?"完成":"选择"}</button>}>
    ...
    <div className={`photo-grid ${selectMode?"select-mode":""}`}>{list.map(i=>{const b=activeBody(state,i) as Photo;return <button data-testid={`photo-${i.id}`} key={i.id} onClick={()=>setSelected(i.id)}><img src={b.src} alt=""/><span>{b.title}</span></button>})}
      {activeDeviceId==="investigation"&&siteUnlocked&&<button data-testid="photo-a3.site.photos" onClick={()=>setSelected("a3.site.photos")}><img src="/media/case-001/placeholders/corridor-frame-417.svg" alt="temporary"/><span>现场照片 · 12</span></button>}
      {activeDeviceId==="investigation"&&videoUnlocked&&<button data-testid="photo-a3.site.video" onClick={()=>setSelected("a3.site.video")}><img src="/media/case-001/placeholders/corridor-frame-417.svg" alt="temporary"/><span>现场视频 · 26.4秒</span></button>}
    </div>
