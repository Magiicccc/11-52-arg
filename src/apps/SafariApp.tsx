...
export function SafariApp(){
  ...
  return <AppChrome title="Safari" actions={<button onClick={()=>setPage("start")}>标签页</button>}>
    <div className="safari-shell">
      ...
      {page==="video"&&<div className="video-page" data-testid="frame-417-page"><img src="/media/case-001/placeholders/corridor-frame-417.svg" alt="第417帧临时素材"/><b>第 417 帧</b><p>服务门旁出现手持钥匙的人影。</p><button className="primary-action" data-testid="inspect-frame-417" onClick={()=>emit("video.frame.inspected","video.corridor.frame417",{frame:417})}>检查这一帧</button>{solved&&<small>FRAME 417 已记录</small>}</div>}
    </div>
  </AppChrome>;
}
