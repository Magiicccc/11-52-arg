...
export function XiaohongshuApp() {
  ...
  if (view==="detail"&&selected) {
    ...
    >
      <article className="xhs-note-detail">
        ...
        <header><button className="xhs-author-button" onClick={()=>setView("profile")}><span className="xhs-avatar">{selected.avatar}</span><b>{selected.author}</b></button><button className={followed?"active":""} onClick={()=>setUiFlag(`xiaohongshu.followed.${selected.author}`,!followed)}>{followed?"已关注":"关注"}</button></header>
        ...
        <section className="xhs-comments">
          ...
          {selected.comments.map(comment=><article key={comment.id}><span className="xhs-avatar">{comment.author.slice(0,1)}</span><div><b>{comment.author}</b><p>{comment.text}</p><small>♡ {comment.likes}</small></div></article>)}
          {playerComments.map((text,index)=><article key={`player-${index}`}><span className="xhs-avatar">川</span><div><b>川流档案</b><p>{text}</p><small>刚刚</small></div></article>)}
        </section>
      </article>
    </DetailShell>;
  }
  ...
  if(view==="profile"&&selected) {
    ...
    return <DetailShell className="xhs-app" title="个人主页" onBack={()=>setView("detail")}>
      <section className="xhs-profile">
        <header><span className="xhs-avatar">{selected.avatar}</span><div><h1>{selected.author}</h1><p>普通生活记录 · {selected.category}</p></div></header>
  ...
  if(tab==="消息") return <XhsTabShell tab={tab} onTab={setTab} onExit={goBack}>
    ...
    <section className="xhs-message-list">{ordinaryXhsNotes.slice(0,8).map((note,index)=><button key={note.id} onClick={()=>openNote(note.id)}><span className="xhs-avatar">{note.avatar}</span><div><b>{note.author}</b><p>{index%2===0?"赞了你的笔记":"回复了你的一条评论"}</p><small>{index+1} 小时前</small></div><img src={assetUrl(note.media)} alt="相关笔记"/></button>)}</section>
  </XhsTabShell>;
  ...
  if(tab==="我") {
    ...
    return <XhsTabShell tab={tab} onTab={setTab} onExit={goBack}>
      <section className="xhs-me-header"><span className="xhs-avatar">川</span><div><h1>川流档案</h1><p>小红书号：chuanliu_archive</p></div><button onClick={()=>setUiFlag("xiaohongshu.profile.editing",true)}>编辑资料</button></section>
    ...
    <div className="platform-scroll xhs-feed" ref={scroll.ref}>
      {notes.map((note, index) => {
        ...
        >
          ...
          <b>{note.title}</b>
          <small><i>{note.avatar}</i> {note.author} <span>♡ {note.likes}</span></small>
        </button>;
      })}
...
export function DouyinApp() {
  ...
  return <div className="app-window platform-app douyin-app" data-testid="douyin-home">
    ...
    <aside className="douyin-actions">
      <button className={followed?"active":""} onClick={()=>{if(!item)return;setUiFlag(`douyin.followed.${item.author}`,!followed);emit("content.item.interacted",item.id,{action:"follow",active:!followed,source:"P"})}}><span className="douyin-avatar">{item?.author.slice(0,1)??"城"}</span><b>{followed?"✓":"＋"}</b></button>
...
export function QQMailApp() {
  ...
  if (view==="detail"&&selected) {
    ...
    return <DetailShell className="qqmail-app" title="邮件" onBack={() => setView("list")} footer={
      ...
      <article className="mail-detail">
        ...
        <header><span className="mail-avatar">{selected.from.slice(0, 1)}</span><div><b>{selected.from}</b><small>发给 沈川 · {selected.date}</small></div><button className={starred?"active":""} onClick={()=>setUiFlag(`qqmail.starred.${selected.id}`,!starred)}>{starred?"★":"☆"}</button></header>
        {selected.body.map((paragraph,index)=><p key={index}>{paragraph}</p>)}
  ...
  if (tab !== "邮件") {
    ...
    return <div className="app-window platform-app qqmail-app" data-testid={`qqmail-${tab}`}>
      <header className="qqmail-header"><button className="mail-account" onClick={() => setTab("邮件")}>沈</button><strong>{tab}</strong><button aria-label="新增" onClick={() => {
  ...
  return <div className="app-window platform-app qqmail-app" data-testid="qqmail-home">
    <header className="qqmail-header"><button aria-label="邮箱文件夹" className="mail-account" onClick={()=>setView("folders")}>沈</button><strong>{folder}</strong><div><button aria-label="搜索" onClick={()=>setView("search")}>⌕</button><button aria-label="写邮件" onClick={()=>setView("compose")}>＋</button></div></header>
...
export function BaiduNetdiskApp() {
  ...
  if (tab !== "首页") {
    ...
    return <div className="app-window platform-app netdisk-app" data-testid={`baidunetdisk-${tab}`}>
      <header className="netdisk-header"><span className="netdisk-account">沈</span><div><b>{tab}</b><small>百度网盘</small></div><button onClick={() => {
  ...
  return <div className="app-window platform-app netdisk-app" data-testid="baidunetdisk-home">
    <header className="netdisk-header"><span className="netdisk-account">沈</span><div><b>百度网盘</b><small>安全保存每一份文件</small></div><button onClick={() => {
      setUiFlag("baidunetdisk.upload.open", true);
      emit("media.picker.opened", "app.files", { surface: "baidunetdisk", source: "P" });
    }}>＋</button></header>
