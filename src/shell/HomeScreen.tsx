...
export function HomeScreen({deviceId}:{deviceId:DeviceId}){
  ...
  return <div className="home-screen" data-testid="home-screen">
    ...
    <div className="home-pages">{pages.map((page,p)=><div className="home-page" key={p}>{page.map(app=><button className="app-icon-button" data-testid={`app-${app.id}`} key={app.id} onClick={()=>openApp(app.id)}><span className="icon-wrap"><img src={assetUrl(app.iconAsset)} alt=""/><i>{state.devices[deviceId].unreadByApp[app.id]||""}</i></span><span>{app.displayName}</span></button>)}</div>)}</div>
